...
def _sparse_attn_chunked(
    ...
    for cs in range(0, max_topk, chunk_topk):
        ...
        # NOTE: a `if not valid.any(): continue` early-out here is a host sync
        # (illegal under FULL cudagraph) and measured ~5-7% slower at long
        # context (the per-chunk sync costs more than the rare all-padding skip
        # saves). Left out; correctness handled by masked_fill below.
...
def flash_mla_with_kvcache_rocm(
    ...
    * ``extra_k_cache`` + ``extra_indices_in_kvcache`` / ``extra_topk_length``
    ...
    def step(idx_chunk: torch.Tensor, cache: torch.Tensor) -> None:
        nonlocal m, l, O
        valid = idx_chunk >= 0
        # NOTE: a `if not valid.any(): return` early-out here is a host sync
        # (illegal under FULL cudagraph) and measured slower at long context.
        # Left out; correctness handled by masked_fill below.
