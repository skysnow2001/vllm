...
def _routing_compute_indx(pid_m, GatherIndx, ScatterIndx, GateScal, ExptScal, ExptIndx, PartialOffs, stride_pm,
    ...
    # tl.arange / tl.sort require a power-of-2 length. N_EXPTS_ACT (top-k) need
    # not be a power of 2 (e.g. DeepSeek-V4 top-k=6 -> 6*BLOCK_M), so pad up to
    # the next power of 2 and mask the padding. Padding lanes load expert=-1,
    # which round-trips to the 0xffff sentinel and is excluded by `mask` below,
    # and they sort to the end so real-lane run-lengths stay correct.
    ...
    expert = tl.load(
        ExptIndx + offs,
        mask=(offs < n_gates) & (local_offs < N_EXPTS_ACT * BLOCK_M),
        other=-1,
    ).to(tl.uint32)
