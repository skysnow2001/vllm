...
# NOTE: DSv4 editable COPY of fusions/fused_reduce_qk_norm_rope_swa_write.py
# Verbatim launcher (kernel import repointed to dsv4_qnorm_rope_kv_insert).
# See "DISPATCH DIFFERENCES vs DSv4" at the bottom for what to change.
...
from aiter.ops.triton._triton_kernels.fusions.dsv4_qnorm_rope_kv_insert import (
...
# ===========================================================================
# DISPATCH DIFFERENCES vs DSv4 (_deepseek_v4_qnorm_rope_kv_insert_reference in
# vllm/models/deepseek_v4/attention.py:141). Change these before wiring:
#
# 1. cos/sin cache: this kernel wants SEPARATE cos_cache/sin_cache, each shaped
#    [max_pos, 1, 1, rope_head_dim//2]. DSv4 has ONE combined cos_sin_cache
#    (cos=[:half], sin=[half:]). Split+reshape before the call, or edit the
#    kernel's cos/sin loads. REUSE_FREQS_FRONT_PART=True assumes half-length freqs.
# 2. RoPE style: DSv4 uses GPT-J interleaved (reshape (half,2), even/odd) ->
#    pass is_neox=False (_get_gptj_rotated_x). VALIDATE it matches numerically.
# 3. Q norm: DSv4 q_head_norm is WEIGHT-FREE -> pass q_norm_weight=None (matches).
# 4. KV FP8 insert: this kernel writes bf16 (in-place kv or bf16 sliding-window
#    swa_kv). DSv4 needs UE8M0 FP8 into the PACKED fp8_ds_mla paged cache via
#    quantize_and_insert_k_cache (slot_mapping + block_size). Run HAS_SWA=False
#    and feed the RoPE'd kv to quantize_and_insert_k_cache SEPARATELY.
# 5. Q head padding: DSv4 output is [T, q_head_padded, head_dim] with zeros in
#    trailing pad heads -> allocate padded q_out and pass it.
# 6. split-K: DSv4 q is already reduced -> pass q 2D [M, num_local_heads*head_dim],
#    num_splitk=1.
# ===========================================================================
