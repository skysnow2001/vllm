...
`MOE_BACKEND=triton_unfused`, `VLLM_ROCM_USE_AITER=1`,
`VLLM_ROCM_USE_AITER_MOE=0`, `CUDAGRAPH_MODE=FULL_DECODE_ONLY`). Paths are
`file:symbol`.
...
| Indexer — prefill MQA-logits | **DeepGEMM** `sparse_attn_indexer.py:233:fp8_fp4_mqa_logits` | AITER `fp8_mqa_logits` (gluon/CK) | **Triton** aiter `fp8_mqa_logits` via `rocm_aiter_mla_sparse.py:rocm_aiter_sparse_attn_indexer` (gluon off) |
...
| O-proj — wo_a grouped GEMM | **DeepGEMM** `fp8_einsum` (FP8) | torch `torch.einsum` (rocBLAS bf16) | **torch `torch.einsum`** (rocBLAS bf16) — same as CDNA; an aiter Triton `batched_gemm_bf16` was tried but is ~1.3–1.8× slower at B=1,K=4096,N=1024 |
| qnorm + RoPE (q side) | C++ `_C.fused_deepseek_v4_qnorm_rope_kv_rope_quant_insert` | **C++/HIP** same `_C` fused kernel | **C++/HIP** same `_C` fused kernel (rebuilt for gfx1201) |
| KV FP8 quant + cache insert | (part of the C++ fused op) | (part of the C++ fused op) | **(part of the C++ fused op)** |
...
| Sampler (top-k / top-p) | **FlashInfer** `flashinfer.sampling` | **CK/HIP** aiter `top_k_top_p_sampling_from_probs` (64-lane wavefront, gfx9x-only) | **native** torch/Triton `v1/sample/ops/topk_topp_sampler.py:forward_native` (aiter sampler skipped in-code on gfx12 via `_ON_GFX12X`, even with AITER on) |
...
compressor, MHC pre/post/fused, HC head, indexer-Q,
q/kv-norm, MTP norms, KV dequant/gather, index plumbing, **MoE experts
(triton_unfused / matmul_ogs)**, **dense FP8 block-scale linears**.

**Still not Triton on gfx12** (work fine natively, just not Triton):
- o-proj inverse-RoPE → **torch** (`forward_native`)
- o-proj wo_a GEMM → **rocBLAS** (`torch.einsum`) — works natively, faster than Triton here
- qnorm+rope+kv-insert → **C++/HIP** `_C` fused kernel (rebuilt for gfx1201; same as CDNA/CUDA)
...
- `VLLM_DSV4_TRITON=1`: forces aiter-Triton blockscale GEMM.
- `VLLM_ROCM_USE_AITER=1` + `VLLM_ROCM_USE_AITER_MOE=0`: AITER on so the sparse
  indexer uses the upstreamed aiter `rocm_aiter_sparse_attn_indexer` op (which
  handles the V4 pre-inserted-K layout and has non-gfx942 Triton paths, gluon
  off); MoE stays on `triton_unfused`. The aiter sampler is gfx9x-only so it's
  skipped in-code on gfx12 (`_ON_GFX12X` guard) → native sampler.
- `MOE_BACKEND=triton_unfused`: OAI `triton_kernels` MoE (modular → DeepSeek-V4
  ...
  `routing_details/_routing_compute.py`. Also needs the **RoutedExperts quant
  fix** (`quant_config.py`, PR #41184 regression) so MXFP4 experts get
  `Mxfp4MoEMethod` instead of falling through to FP8.
